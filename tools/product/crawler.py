...
def level_two():
    ...
    for i in level_one_tuple:
        code = i.get('code')
        sql_id = i.get('id')
        html_data = extract_item_by_re(get_response(url=request_url % code, encoding='gb2312', headers=header).text,
...
def level_three():
    ...
    for i in level_one_tuple:
        ...
        for j in level_two_tuple:
            code_two = j.get('code')
            id_two = j.get('id')
            html_data = extract_item_by_re(get_response(url=request_url_three % (code_one, code_two), encoding='gb2312', headers=header).text,
...
def level_four():
    request_url_four = 'http://www.stats.gov.cn/tjsj/tjbz/tjypflml/2010/%s/%s/%s.html'
    level_one_tuple = query(
        sql=u"SELECT `id`, `code` FROM product WHERE level = 1")
    for i in level_one_tuple:
        code_one = i.get('code')
...
def level_five():
    request_url_five = 'http://www.stats.gov.cn/tjsj/tjbz/tjypflml/2010/%s/%s/%s/%s.html'
    level_one_tuple = query(
        sql=u"SELECT `id`, `code` FROM product WHERE level = 1")
    for i in level_one_tuple:
        code_one = i.get('code')
