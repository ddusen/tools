...
def get_industry():
    ...
    for ids in level_2_ids:
        ...
        for l3 in level_3:
            ...
            industry_inspection_count = query(
                sql=u"SELECT COUNT(*) FROM yqj2.origin_inspection_industry WHERE industry_id = %s " % level_3_id)
            level_3_industry_count[level_3_name] = int(
                industry_inspection_count[0].get('COUNT(*)'))
...
def write_excel(industry_count_data):
    ...
    # 保存文件
    file.save('level3_industry_inspection_count.xls')
